...
if [ $TRAVIS_PULL_REQUEST = "false" ] && [ $TRAVIS_BRANCH = $DEPLOY_BRANCH ]; then
  ...
  scp -i rdr2018 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -rp _site/* ${SSH_USER}@${SSH_PATH}
elif [ $TRAVIS_PULL_REQUEST = "false" ] && [ $TRAVIS_BRANCH = $STAGING_BRANCH ]; then
  scp -i rdr2018 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -rp _site/* ${SSH_USER}@${SSH_STG_PATH}
else
